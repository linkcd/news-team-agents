...
│   │   ├── app/NewsCollector/      # Production agent code (deployed to container)
...
│   │   ├── src/                    # Development source (mirrors app/, used for local tests)
...
│   └── Dockerfile              # Generated by agentcore CLI — do NOT modify
├── src/                        # Dev mirror of app/ (for local pytest with mocked strands)
└── tests/                      # Unit tests (run with python3 -m pytest)
```
...
- **`src/` vs `app/`**: `src/` is for local development and pytest (uses mocked strands SDK). `app/` is the production code that gets deployed. Keep them in sync.
- **Tests mock `strands` and `boto3`** in `conftest.py` since they aren't installed in the test environment. The mock makes `@tool` a passthrough decorator.
