...
Without a ping_handler, the AgentCore control plane always sees HEALTHY and routes
concurrent requests to the same instance, causing ConcurrencyException. The fix is
to provide a ping_handler that returns HEALTHY_BUSY when the agent's invocation lock
is held.
"""
...
def test_serve_a2a_called_with_ping_handler():
    ...
    main_path = os.path.join(
        os.path.dirname(__file__), "..", "app", "NewsCollector", "main.py"
    )
